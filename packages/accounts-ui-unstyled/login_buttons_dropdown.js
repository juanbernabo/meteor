(function () {
  ...
  Template.loginButtonsLoggedOutDropdown.hasPasswordService = function () {
    return Meteor._loginButtons.hasPasswordService();
  };
  ...
  Template.loginButtonsLoggedOutAllServices.hasOtherServices = function () {
    return Meteor._loginButtons.getLoginServices().length > 1;
  };
  ...
  Template.loginButtonsLoggedOutPasswordService.fields = function () {
    ...
    var fields = loginButtonsSession.get('inSignupFlow') ? signupFields : loginFields;
  ...
  var signup = function () {
    ...
    var username = elementValueById('login-username');
    if (username !== null) {
      if (!Meteor._loginButtons.validateUsername(username))
        return;
      else
    ...
    var email = elementValueById('login-email');
    if (email !== null) {
      if (!Meteor._loginButtons.validateEmail(email))
        return;
      else
        options.email = email;
    }
